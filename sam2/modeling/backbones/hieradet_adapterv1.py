...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        if self.use_adapter:
            self.adapter = Adapter(input_dim=dim_out, bottleneck_dim=adapter_bottleneck_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # --- 核心修改：forward方法完全恢复到和原始hieradet.py一模一样！ ---
        shortcut = x
        x = self.norm1(x)
...
class Hiera(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        # =================== 核心调试修改：在这里加入打印 ===================
        # print("\n" + "="*20 + " Starting Hiera Forward Pass " + "="*20)
        
        # for i, blk in enumerate(self.blocks):
            
        #     # 打印当前Block的输入形状
        #     print(f"--- Entering Block {i} ---")
        #     print(f"  Input shape : {x.shape}")
        #     print(f"  Block properties: dim={blk.dim}, dim_out={blk.dim_out}, q_stride={blk.q_stride}, window_size={blk.window_size}")
        ...
        #         i in self.stage_ends and self.return_interm_layers
        #     ):
        #         print(f"  >> Storing output feature from Block {i}")
        #         feats = x.permute(0, 3, 1, 2)
        #         outputs.append(feats)

        # print("="*20 + " Finished Hiera Forward Pass " + "="*20 + "\n")
        # =================== 调试修改结束 =================================
