...
class Trainer:
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                if data_iter % self.logging_conf.log_scalar_frequency == 0:
                    ###新加的if
                    if self.optim.schedulers is not None:
    ...
    def _setup_components(self):
        ...
        self.trainable_param_names = None # 初始化一个实例变量

        use_adapter_finetuning = self.model_conf.get("use_adapter", False)

        if use_adapter_finetuning:
            print("\n" + "="*50)
            print("ADAPTER PEFT MODE ACTIVATED")
            print("="*50)

            # 1. 冻结所有原始参数
            print("Freezing all original model parameters...")
            for name, param in self.model.named_parameters():
                param.requires_grad = False
            
            # 2. 解冻Adapter参数并收集它们的【名字】
            trainable_param_names_list = []
            print("Unfreezing Adapter parameters for training...")
            for name, param in self.model.named_parameters():
                if "adapter" in name.lower():
                    param.requires_grad = True
                    trainable_param_names_list.append(name)
                    print(f"  - [WILL TRAIN] {name}")
            
            # 将名字列表转换为集合，存为实例变量，供下一步使用
            self.trainable_param_names = set(trainable_param_names_list)
            print(f"Total {len(self.trainable_param_names)} adapter parameter tensors will be trained.")
            print("="*50 + "\n")
    ...
    def _construct_optimizers(self):
        self.optim = construct_optimizer(
            ...
            #2025/9/20
            param_allowlist=self.trainable_param_names # 传递我们准备好的“白名单”
            #2025/9/20
        )
